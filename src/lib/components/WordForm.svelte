<script lang="ts">
  ...
  function removeAffix(a: string) { form.affixes = form.affixes.filter(x => x !== a); }
  function affixKeydown(e: KeyboardEvent) {
    if (e.key === 'Enter') { e.preventDefault(); addAffix(); }
  }
  ...
  function removeSpell(s: string) { form.spellings = form.spellings.filter(x => x !== s); }
  function spellKeydown(e: KeyboardEvent) {
    if (e.key === 'Enter') { e.preventDefault(); addSpell(); }
  }
</script>
...
<div class="wf">
  ...
  <div class="form-row fr-2">
    <div class="fg">
      ...
      <input id="wf-name" class="fi" class:err={errors.name} bind:value={form.name}
        placeholder="e.g. kliri" autocomplete="off" />
  ...
  <div class="form-row fr-4">
    <div class="fg"><label for="wf-source">Source</label>
      <input id="wf-source" class="fi" bind:value={form.source} placeholder="JCB, L4…" /></div>
    <div class="fg"><label for="wf-year">Year</label>
      <input id="wf-year" class="fi" bind:value={form.year} placeholder="'75" /></div>
    <div class="fg"><label for="wf-rank">Rank</label>
      <input id="wf-rank" class="fi" bind:value={form.rank} placeholder="1.0" /></div>
    <div class="fg"><label for="wf-match">Match %</label>
      <input id="wf-match" class="fi" bind:value={form.match_} placeholder="49%" /></div>
  </div>
  ...
  <div class="form-row fr-origin">
    <div class="fg" style="flex:3">
      ...
      <input id="wf-origin" class="fi" bind:value={form.origin} placeholder="3/4E clear | 4/5J kirei…" />
    </div>
    ...
    <div class="fg">
      ...
      <div class="tag-field">
        {#each form.affixes as a}
          <span class="badge bd-afx">{a}
            <button class="tag-rm" onclick={() => removeAffix(a)}>×</button>
          </span>
        {/each}
        <input id="wf-affix-inp" class="fi tag-inp" bind:value={newAffix}
          placeholder="add…" onkeydown={affixKeydown} />
    ...
    <div class="fg">
      ...
      <div class="tag-field">
        {#each form.spellings as s}
          <span class="badge bd-spell">{s}
            <button class="tag-rm" onclick={() => removeSpell(s)}>×</button>
          </span>
        {/each}
        <input id="wf-spell-inp" class="fi tag-inp" bind:value={newSpell}
          placeholder="add…" onkeydown={spellKeydown} />
...
<style>
  ...
  .fr-origin{ display:flex;gap:.5rem;margin-bottom:.44rem }

  /* error */
  .fi.err{ border-color:var(--red-d) }
  .errtxt{ font-size:.58rem;color:var(--red);margin-top:.1rem }
  ...
  .tag-field:focus-within{ border-color:var(--gold-d) }

  .tag-inp{
    flex:1;min-width:40px;height:20px;
    background:transparent;border:none;outline:none;
    font-family:inherit;font-size:.68rem;color:var(--text);
    padding:0;
  }
  ...
  .tag-rm{
    ...
    padding:0;font-size:.72rem;opacity:.6;line-height:1;
  }
  .tag-rm:hover{ opacity:1 }

  /* responsive: stack 4-col to 2-col on narrow */
  @media(max-width:520px){
    .fr-4{ grid-template-columns: 1fr 1fr }
    .fr-2{ grid-template-columns: 1fr }
    .fr-origin{ flex-direction:column }
  }
</style>
