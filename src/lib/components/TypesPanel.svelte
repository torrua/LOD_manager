<script lang="ts">
  ...
  function startNew() { form={name:'',type_x:'',group_:''}; creating=true; editing=null; }
  function cancel()   { editing=null; creating=false; }
  async function submit() {
    if (!form.name.trim()) return;
    await saveType(editing, { name:form.name, type_x:form.type_x||null, group_:form.group_||null });
    editing=null; creating=false;
  }
  // issue #7: respond to topbar + New signal
  $effect(() => { if (app.newSignal > 0 && app.tab==='types') startNew(); });
</script>
            ...
            {#if !app.readonly}
              <td>
                <div class="row-acts edit-only">
                  <button class="btn btn-icon btn-sm btn-ghost" onclick={() => startEdit(t.id)}>✎</button>
                  <button class="btn btn-icon btn-sm btn-ghost btn-r" onclick={() => deleteType(t.id)}>🗑</button>
                </div>
              </td>
            {/if}
          </tr>
...
{#snippet typeForm(title: string)}
  <div class="inline-form">
    ...
    <div class="form-row">
      <div class="fg"><label for="tp-name">Name *</label><input id="tp-name" class="fi" bind:value={form.name} placeholder="C-Prim" /></div>
      <div class="fg"><label for="tp-abbr">Abbreviation</label><input id="tp-abbr" class="fi" bind:value={form.type_x} placeholder="CP" /></div>
      <div class="fg"><label for="tp-group">Group</label><input id="tp-group" class="fi" bind:value={form.group_} placeholder="Primitive" /></div>
    </div>
