...
<div class="el-root">
  ...
  {:else if results.length > 0}
    {#each results as r (r.word_id)}
      <div class="el-row" class:on={app.curWord?.id === r.word_id}
...
<style>
  .el-root{display:flex;flex-direction:column;padding:.1rem 0}
  .el-status{padding:1.2rem .5rem;font-size:.72rem;color:var(--sb-text2);text-align:center}
  .el-status.empty span{color:var(--gold)}

  .el-row{
    padding:.32rem .48rem .28rem .42rem;
    border-left:2px solid transparent;
    cursor:pointer;
    transition:background 100ms,border-color 100ms;
    -webkit-tap-highlight-color:transparent;
  }
  .el-row:hover{background:var(--sb-hover)}
  .el-row.on{background:var(--gold-g);border-left-color:var(--gold)}

  .el-top{display:flex;align-items:baseline;gap:.25rem;flex-wrap:wrap;margin-bottom:.1rem}
  .el-name{font-size:.73rem;font-weight:600;color:var(--sb-text)}
  .el-row.on .el-name{color:var(--gold)}
  .el-type{font-size:.5rem;color:var(--sb-text2);background:rgba(128,128,128,.1);
    padding:1px 4px;border-radius:2px}
  .el-gram{font-size:.55rem;color:var(--teal)}
  .el-cnt{font-size:.5rem;color:var(--blue);background:rgba(106,154,200,.1);
    border:1px solid rgba(106,154,200,.25);padding:0 3px;border-radius:2px;
    margin-left:auto}
  ...
  /* highlighted match */
  :global(.el-snip mark){
    background:transparent;color:var(--teal);font-weight:600;font-style:italic;
  }

  .el-more{font-size:.6rem;color:var(--sb-text2);padding:.5rem .5rem .8rem;
    text-align:center;border-top:1px solid var(--sb-border)}

  @media(max-width:640px){
    .el-row{padding:.4rem .5rem}
    .el-name{font-size:.82rem}
    .el-snip{font-size:.72rem}
  }
</style>
