<script lang="ts">
  ...
  function cancel() { app.editing = false; app.panel = ev ? 'event' : 'welcome'; if (!ev) app.mobileShowList = true; }
</script>
...
<style>
  ...
  .form-row.c2{grid-template-columns:repeat(auto-fit,minmax(130px,1fr))}
</style>
