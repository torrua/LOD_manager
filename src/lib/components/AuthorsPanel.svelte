<script lang="ts">
  ...
  function startEdit(id: number) {
    ...
    editing=id; creating=false;
  }
  function startNew() { form={initials:'',full_name:'',notes:''}; creating=true; editing=null; }
  function cancel()   { editing=null; creating=false; }
  async function submit() {
    if (!form.initials.trim()) return;
    await saveAuthor(editing, { initials:form.initials, full_name:form.full_name||null, notes:form.notes||null });
    editing=null; creating=false;
  }
  // issue #7: respond to topbar + New signal
  $effect(() => { if (app.newSignal > 0 && app.tab==='authors') startNew(); });
</script>
            ...
            {#if !app.readonly}
              <td>
                <div class="row-acts edit-only">
                  <button class="btn btn-icon btn-sm btn-ghost" onclick={() => startEdit(a.id)}>✎</button>
                  <button class="btn btn-icon btn-sm btn-ghost btn-r" onclick={() => deleteAuthor(a.id)}>🗑</button>
                </div>
              </td>
            {/if}
          </tr>
...
{#snippet authorForm(title: string)}
  <div class="inline-form">
    ...
    <div class="form-row">
      <div class="fg"><label for="ap-initials">Initials *</label><input id="ap-initials" class="fi" bind:value={form.initials} placeholder="JCB" /></div>
      <div class="fg"><label for="ap-fname">Full name</label><input id="ap-fname" class="fi" bind:value={form.full_name} placeholder="James Cooke Brown" /></div>
    </div>
    <div class="fg" style="margin-bottom:.42rem">
      <label for="ap-notes">Notes / comments</label>
      <textarea id="ap-notes" class="fta" bind:value={form.notes} rows="2" placeholder="Optional notes about this author…"></textarea>
    </div>
