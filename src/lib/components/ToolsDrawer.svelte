<script lang="ts">
  ...
  async function pickImport() {
    const sel = await openFilePicker({ multiple:true, title:'Select LOD text files',
      filters:[{name:'Text',extensions:['txt']}] });
    if (sel) { impPaths=Array.isArray(sel)?sel:[sel]; impResult=null; }
  }
  function rmImp(p: string) { impPaths = impPaths.filter(x=>x!==p); }
  function bn(p: string) { return p.split(/[/\\]/).pop()||p; }
  async function runImport() {
    if (!impPaths.length||impRunning) return;
    impRunning=true; impResult=null;
    try {
      impResult = await importFiles(impPaths) as ImportResult;
      toast(`Import: ${impResult.words} words`, impResult.errors?'err':'ok');
    } catch(e) { toast(String(e),'err'); } finally { impRunning=false; }
  }
  ...
  async function runExport() {
    ...
    finally { expRunning=false; }
  }

  // ── Database ──────────────────────────────────────────────────────────────
  async function handleSwitch() {
    const p = await openDbDialog({ title:'Open LOD Database',
      filters:[{name:'SQLite',extensions:['db','sqlite','sqlite3']}] });
    if (p) await openDb(p as string).catch(e=>toast(String(e),'err'));
  }
  async function handleNew() {
    const p = await saveDialog({ title:'Create New LOD Database',
      defaultPath:'loglan.db', filters:[{name:'SQLite',extensions:['db']}] });
...
<button class="td-backdrop" onclick={() => app.toolsOpen=false} aria-label="Close tools"></button>
...
<div class="td">
  ...
  <div class="td-body">
    ...
    {#if app.toolsTab==='database'}
      {#if app.dbOpen}
        <div class="db-stat-grid">
          ...
          {#if app.dbStats}
            <span class="dsl">Words</span>     <span class="dsv">{app.dbStats.word_count.toLocaleString()}</span>
            <span class="dsl">Definitions</span><span class="dsv">{app.dbStats.definition_count.toLocaleString()}</span>
            ...
            {#if app.dbStats.settings.length>0}
              <span class="dsl">Settings</span><span class="dsv">{app.dbStats.settings.length} keys</span>
    ...
    {:else if app.toolsTab==='import'}
      <p class="td-hint">Select LOD text files: <code>Word.txt</code>, <code>WordDef.txt</code>,
        <code>WordSpell.txt</code>, <code>LexEvent.txt</code>, <code>type.txt</code>,
        <code>author.txt</code>, <code>settings.txt</code>.</p>
      ...
      {#if impPaths.length>0}
        <button class="btn btn-g btn-sm" style="margin-top:.5rem" onclick={runImport} disabled={impRunning||!app.dbOpen}>
          {impRunning?'Importing…':`Import ${impPaths.length} file${impPaths.length>1?'s':''}`}
        </button>
      {/if}
      {#if !app.dbOpen}
      ...
      {#if impResult}
        <div class="imp-res" class:err={impResult.errors>0}>
          ...
          Words: {impResult.words} · Defs: {impResult.definitions} ·
          Events: {impResult.events} · Settings: {impResult.settings} ·
    ...
    {:else if app.toolsTab==='settings'}
      ...
      <div class="settings-group">
        ...
        {#if app.prefs.visibleMeta.length === 0}
          <p class="td-hint td-warn" style="margin-top:.3rem">No fields selected — metadata hidden.</p>
        {/if}
      </div>
      ...
      <div class="settings-group">
        ...
        <div class="ck-indent" class:ck-disabled={!app.prefs.elShowDetails}>
          <label class="ck-row">
            <input type="checkbox" class="ck" disabled={!app.prefs.elShowDetails}
      ...
      <div class="settings-group">
        ...
        <p class="td-hint" style="margin:.2rem 0">FTS5 is faster and supports prefix / phrase matching.</p>
...
<style>
  ...
  .td{position:fixed;right:0;top:0;bottom:0;width:310px;max-width:94vw;
    ...
    display:flex;flex-direction:column;box-shadow:-4px 0 24px var(--shd-lg);
    animation:td-in 160ms ease}
  @keyframes td-in{from{transform:translateX(100%)}to{transform:translateX(0)}}
  @media(max-width:640px){
    .td{right:0;left:0;top:auto;bottom:0;width:100%;max-width:100%;
      border-left:none;border-top:1px solid var(--border);
      border-radius:14px 14px 0 0;max-height:90vh;min-height:50vh;
      animation:td-up 200ms ease}
    @keyframes td-up{from{transform:translateY(100%)}to{transform:translateY(0)}}
  }
  .td-hdr{display:flex;align-items:center;justify-content:space-between;
    padding:.55rem .85rem .4rem;border-bottom:1px solid var(--border);flex-shrink:0}
  .td-title{font-size:.75rem;font-weight:700;color:var(--text);letter-spacing:.04em}
  .td-tabs{display:flex;gap:2px;padding:.24rem .36rem .16rem;
    border-bottom:1px solid var(--border);flex-shrink:0}
  .td-tab{flex:1;height:26px;font-size:.58rem;font-weight:600;letter-spacing:.04em;
    ...
    transition:all 140ms;padding:0}
  .td-tab:hover:not(.on){background:var(--surf2);border-color:var(--border)}
  .td-tab.on{color:var(--gold);border-color:var(--gold-d);background:var(--gold-g)}
  .td-body{flex:1;overflow-y:auto;padding:.75rem .85rem;display:flex;flex-direction:column;gap:.55rem}
  .td-field-label{font-size:var(--fs-label);color:var(--text3);text-transform:uppercase;
    letter-spacing:.09em;margin-bottom:.22rem;display:block}
  .td-hint{font-size:.7rem;color:var(--text2);line-height:1.6}
  .td-hint code{color:var(--gold);background:var(--gold-g);padding:0 3px;border-radius:2px}
  .td-warn{color:var(--red);margin-top:.3rem}
  .td-acts{display:flex;flex-wrap:wrap;gap:.32rem}

  /* issue #2: stats grid */
  .db-stat-grid{display:grid;grid-template-columns:max-content 1fr;gap:.12rem .6rem;
    background:var(--surf2);border:1px solid var(--border);border-radius:var(--r-lg);
    padding:.6rem .75rem;margin-bottom:.45rem}
  .dsl{font-size:.52rem;color:var(--text3);text-transform:uppercase;letter-spacing:.07em;
    display:flex;align-items:center}
  .dsv{font-size:.73rem;color:var(--text);font-weight:500;overflow:hidden;
    text-overflow:ellipsis;white-space:nowrap;display:flex;align-items:center}
  .dsl-sub{font-size:.46rem;padding-left:.5rem;color:var(--text3)}
  .dsv-sub{font-size:.63rem;color:var(--text2)}

  /* export direction */
  .dir-row{display:flex;gap:.28rem;margin-top:.2rem}
  .dir-btn{flex:1;height:28px;font-size:.65rem;font-family:inherit;border-radius:var(--r-md);
    background:var(--surf2);border:1px solid var(--border);color:var(--text2);cursor:pointer;
    transition:all 140ms}
  .dir-btn.on{background:var(--gold-g);border-color:var(--gold-d);color:var(--gold)}
  .dir-btn:hover:not(.on){background:var(--surf3);border-color:var(--border3)}

  /* import */
  .file-zone{border:1px dashed var(--border2);border-radius:var(--r-lg);padding:.5rem .6rem;
    display:flex;flex-direction:column;gap:.2rem;min-height:48px;justify-content:center;align-items:flex-start}
  .file-zone.has{align-items:stretch}
  .file-row{display:flex;align-items:center;gap:.35rem;background:var(--surf2);
    border:1px solid var(--border);border-radius:var(--r-sm);padding:.16rem .38rem}
  .f-name{font-size:.68rem;color:var(--gold);font-weight:600;flex:1;
    overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .imp-res{font-size:.7rem;padding:.42rem .55rem;border-radius:var(--r-md);
    background:var(--green-g);border:1px solid var(--green-d);color:var(--green)}
  .imp-res.err{background:var(--red-g);border-color:var(--red-d);color:var(--red)}

  /* settings */
  .ck-indent{margin-left:1.3rem;display:flex;flex-direction:column;gap:.05rem}
  .ck-disabled{opacity:.4;pointer-events:none}
  .settings-group{background:var(--surf2);border:1px solid var(--border);
    border-radius:var(--r-lg);padding:.6rem .75rem}
  .sg-title{font-size:.58rem;font-weight:700;color:var(--text3);
    text-transform:uppercase;letter-spacing:.08em;margin-bottom:.42rem}
  .ck-row{display:flex;align-items:center;gap:.45rem;font-size:.73rem;color:var(--text2);
    padding:.22rem 0;cursor:pointer;user-select:none}
  .ck-row:hover{color:var(--text)}
  .ck{accent-color:var(--gold);width:13px;height:13px;flex-shrink:0;cursor:pointer}
</style>
