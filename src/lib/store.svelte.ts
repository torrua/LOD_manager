...
import type { WordListItem, WordDetail, EventItem, TypeItem, AuthorItem, AppInfo, DbStats, ELResult, SearchMode, Tab } from '../types';

// ─── Preferences ─────────────────────────────────────────────────────────────
function loadPrefs() {
  try { return JSON.parse(localStorage.getItem('lod-prefs') || '{}'); } catch { return {}; }
}
function savePrefs() { localStorage.setItem('lod-prefs', JSON.stringify(app.prefs)); }

const DEFAULT_META = ['type','source','year','rank','match','event'];

export const app = $state({
  dbOpen: false, dbPath: '', wordCount: 0,
  theme: (localStorage.getItem('lod-theme') || 'dark') as 'dark' | 'light',
  readonly: localStorage.getItem('lod-ro') === '1',
  tab: 'words' as Tab,
  words: [] as WordListItem[], filteredWords: [] as WordListItem[],
  searchQ: '', typeFilter: '',
  curWord: null as WordDetail | null, curEvent: null as EventItem | null,
  types: [] as TypeItem[], authors: [] as AuthorItem[], events: [] as EventItem[],
  panel: 'welcome' as string, editing: false,
  history: [] as Array<{ tab: Tab; id: number }>, historyIdx: -1,
  toolsOpen: false, toolsTab: 'database' as 'import' | 'export' | 'database' | 'settings',
  newSignal: 0,
  mobileShowList: true,
...
export function toast(msg: string, kind: 'ok' | 'err' | 'info' = 'ok') {
  ...
  app.toast = { msg, kind };
  app.toastTimer = setTimeout(() => { app.toast = null; }, 2800) as unknown as number;
}

export async function openDb(path: string) {
  const info: AppInfo = await invoke('open_database', { path });
  app.dbOpen = true; app.dbPath = info.db_path;
  localStorage.setItem('lod-last-db', path);
  await loadAll();
  app.panel = 'welcome'; app.toolsOpen = false;
...
export async function createDb(path: string) {
  ...
  app.dbOpen = true; app.dbPath = info.db_path;
  localStorage.setItem('lod-last-db', path);
  await loadAll(); app.panel = 'welcome'; app.toolsOpen = false;
  toast('New database created', 'ok');
}
export function getLastDbPath(): string { return localStorage.getItem('lod-last-db') || ''; }
export function closeDb() {
  app.dbOpen = false; app.dbPath = ''; app.wordCount = 0;
  app.words = []; app.filteredWords = []; app.types = []; app.authors = []; app.events = [];
  app.curWord = null; app.curEvent = null; app.panel = 'welcome'; app.toolsOpen = false;
  app.mobileShowList = true; app.editing = false; app.tab = 'words';
  app.history = []; app.historyIdx = -1;
  app.dbStats = null; localStorage.removeItem('lod-last-db');
}
...
export function applyFilter() {
  ...
  if (q) {
    if (q.includes('*') || q.includes('?')) {
      const pat = new RegExp('^'+q.replace(/[.+^${}()|[\]\\]/g,'\\$&').replace(/\*/g,'.*').replace(/\?/g,'.')+'$','i');
      ws = ws.filter(w => pat.test(w.name));
    } else { ws = ws.filter(w => w.name.toLowerCase().startsWith(q)); }
  }
  if (tf) {
    if (tf.startsWith('__g__')) { const g=tf.slice(5); ws=ws.filter(w=>app.types.find(t=>t.name===w.type_name)?.group_===g); }
    else { ws=ws.filter(w=>w.type_name===tf); }
  }
...
export async function selectWord(id: number, pushHist = true) {
  ...
  try {
    ...
    app.editing = false; app.panel = 'word'; app.tab = 'words';
    app.mobileShowList = false;
    if (pushHist) _pushHistory({ tab: 'words', id });
    _scrollSidebarTo(id);
  } catch(e) { toast('Word not found', 'err'); }
}
export async function saveWord(id: number | null, data: object) {
  ...
  app.curWord = w; app.editing = false; app.panel = 'word'; await loadWords();
}
export async function deleteWord(id: number) {
  await invoke('delete_word', { id }); toast('Deleted', 'ok');
  app.curWord = null; app.panel = 'welcome'; app.mobileShowList = true; await loadWords();
}
export async function saveDef(id: number | null, wordId: number, data: object) {
  app.curWord = await invoke('save_definition', { id, wordId, data }); toast(id?'Updated':'Added','ok');
}
export async function deleteDef(id: number, wordId: number) {
  app.curWord = await invoke('delete_definition', { id, wordId }); toast('Deleted','ok');
}

export async function loadEvents() { app.events = await invoke('get_events'); }
export async function selectEvent(id: number, pushHist = true) {
  app.curEvent = app.events.find(e => e.id === id) || null;
  app.editing = false; app.panel = 'event'; app.tab = 'events';
  app.mobileShowList = false;
...
export async function saveEvent(id: number | null, data: object) {
  ...
  toast(id?'Saved!':'Created!','ok'); await loadEvents();
  app.curEvent = app.events.find(e => e.id === ev.id) || null;
  app.editing = false; app.panel = 'event';
}
export async function deleteEvent(id: number) {
  await invoke('delete_event', { id }); toast('Deleted','ok');
  app.curEvent = null; app.panel = 'welcome'; app.mobileShowList = true; await loadEvents();
}

export async function loadTypes()   { app.types   = await invoke('get_types'); }
export async function saveType(id: number | null, data: object) {
  app.types = await invoke('save_type', { id, data }); toast(id?'Updated!':'Created!','ok');
}
export async function deleteType(id: number) {
  app.types = await invoke('delete_type', { id }); toast('Deleted','ok');
}

export async function loadAuthors() { app.authors = await invoke('get_authors'); }
export async function saveAuthor(id: number | null, data: object) {
  app.authors = await invoke('save_author', { id, data }); toast(id?'Updated!':'Added!','ok');
}
export async function deleteAuthor(id: number) {
  app.authors = await invoke('delete_author', { id }); toast('Deleted','ok');
}

export async function importFiles(paths: string[]) {
  const result = await invoke('import_lod_files', { paths });
  await loadAll(); loadDbStats().catch(()=>{});
  return result;
}
...
export function searchEnglishDebounced(q: string) {
  ...
  if (!q.trim()) { app.elResults = []; return; }
...
export async function goBack() {
  ...
  if (app.tab === 'types' || app.tab === 'authors') {
    ...
    app.mobileShowList = true; return;
  }
...
export function canGoForward() { return app.historyIdx < app.history.length - 1; }
...
function _scrollSidebarTo(id: number) {
  requestAnimationFrame(() => {
    ...
    if (item) { item.scrollIntoView({ block: 'nearest' }); }
      ...
      if (idx >= 0) {
        const ROW_H = 28, top = idx * ROW_H, ch = list.clientHeight;
