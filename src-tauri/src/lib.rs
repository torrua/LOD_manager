...
fn open_database(state: Db, path: String) -> Res<AppInfo> {
    ...
    // One-time data migrations (safe to call every open — each checks own flag)
...
fn import_lod_files(state: Db, paths: Vec<String>) -> Res<ImportResult> {
    ...
    // Rebuild FTS index after import (ignore errors)
    let _ = with_db(&state, |conn| db::rebuild_fts(conn));
    Ok(result)
}
    ...
    if params.use_like {
        ...
        with_db(&state, |conn| {
            match db::search_english_fts(conn, &params.query, params.limit) {
                ...
                // FTS may fail on bad query syntax — fall back to LIKE
...
#[tauri::command]
fn export_html(state: Db, event_name: Option<&str>) -> Res<String> {
    with_db(&state, |conn| {
        export::generate_html(conn, event_name)
    })
}

/// Smoke-test: open an in-memory database, run `init_schema`, insert a word.
#[tauri::command]
fn export_html_to_file(state: Db, path: &str, event_name: Option<&str>) -> Res<()> {
    with_db(&state, |conn| {
        export::write_html_to_file(conn, path, event_name)
    })
}
