...
pub fn import_files(conn: &mut Connection, paths: &[String]) -> ImportResult {
    ...
    if let Some(p) = type_file {
        match fs::read_to_string(&p) {
            ...
            Err(e) => {
                result.errors += 1;
                result.messages.push(format!("Types error: {e}"));
            }
        }
    }
    ...
    if let Some(p) = author_file {
        match fs::read_to_string(&p) {
            ...
            Err(e) => {
                result.errors += 1;
                result.messages.push(format!("Authors error: {e}"));
            }
        }
    }
        ...
        match fs::read_to_string(&p) {
            Ok(content) => {
                for r in rows(&content) {
                    ...
                    if res.is_ok() {
                        let eid: i64 = tx
                            .query_row("SELECT id FROM events WHERE name=?1", params![name], |r| {
                                r.get(0)
                            })
                            .unwrap_or(0);
                ...
                let _ = tx
                    .query_row("SELECT id FROM events WHERE name='Start'", [], |r| {
                        r.get::<_, i64>(0)
                    })
            ...
            Err(e) => {
                result.errors += 1;
                result.messages.push(format!("Events error: {e}"));
            }
        }
    }
        ...
        match fs::read_to_string(&p) {
            Ok(content) => {
                for r in rows(&content) {
                    ...
                    let affixes: Vec<String> = affixes_s
                        .split_whitespace()
                        .filter(|s| !s.is_empty())
                        .map(|s| s.to_string())
                        .collect();
            ...
            Err(e) => {
                result.errors += 1;
                result.messages.push(format!("Word.txt error: {e}"));
            }
        }
    }
        ...
        match fs::read_to_string(&p) {
            Ok(content) => {
                ...
                for r in rows(&content) {
                    ...
                    let ev_end: Option<i64> = ev_end_old
                        .as_deref()
                    ...
                    let (type_name, match_, source, year, rank, origin, origin_x, affixes) =
                        if let Some(st) = word_staging.get(old_id) {
                            st.clone()
                        } else {
                            (String::new(), None, None, None, None, None, None, vec![])
                        };
            ...
            Err(e) => {
                result.errors += 1;
                result.messages.push(format!("WordSpell error: {e}"));
            }
        }
    }
        ...
        match fs::read_to_string(&p) {
            Ok(content) => {
                ...
                for r in rows(&content) {
                    ...
                    let word_id: Option<i64> = old_to_new
                        .get(old_word_id)
                        .copied()
            ...
            Err(e) => {
                result.errors += 1;
                result.messages.push(format!("Definitions error: {e}"));
            }
        }
    }
    ...
    match tx.commit() {
        ...
        Err(e) => {
            result.errors += 1;
            result.messages.push(format!("Commit error: {e}"));
        }
    }
    ...
    if let Some(p) = &settings_file {
        match import_settings(conn, p) {
            ...
            Err(e) => {
                result.errors += 1;
                result.messages.push(format!("settings: {}", e));
            }
        }
    }
